...
<body>
    ...
    <br>
    <h1 align = center>Extro Information</h1>
    <br>
    <table id = 'box' width="50%" border="1" align = center >
            <tr>
                <td id = 'title' style ="border-radius: 10px 10px 0 0;">Extro Information</td>
                <?php
                $sql = "SELECT * FROM extro_info";
                $result = mysqli_query($conn, $sql);
                while($row = mysqli_fetch_array($result)){
                    $filtered = array(
                        'extro_str'=>htmlspecialchars($row['extro_str'])
                    );
                    ?>
                    <tr>
                        <td id = 'fill' ><?=$filtered['extro_str']?></td>
                    </tr> 
                    <?php
                }
                ?>
