...
<body>
    ...
    <br>
    <h1 align = center>Intro Information</h1>
    <br>
    <table id = 'box' width="50%" border="1" align = center >
            <tr>
                <td id = 'title' style ="border-radius: 10px 10px 0 0;">Intro Information</td>
                <?php
                $sql_i = "SELECT * FROM intro_info";
                $result_i = mysqli_query($conn, $sql_i);
                while($row = mysqli_fetch_array($result_i)){
                    $filtered = array(
                        'intro_str'=>htmlspecialchars($row['intro_str'])
                    );
                    ?>
                    <tr>
                        <td id = 'fill' ><?=$filtered['intro_str']?></td>
                    </tr> 
                    <?php
                }
                ?>
